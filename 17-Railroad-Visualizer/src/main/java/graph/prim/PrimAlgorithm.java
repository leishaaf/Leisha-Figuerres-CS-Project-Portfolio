...
public class PrimAlgorithm extends MSTAlgorithm {
    ...
    public void computeMST() {
        ...
        for(int i = 0; i < numVertices(); i++){ // initializing table, each array for each column in table
            this.added[i] = false;
        ...
        while(addedCount < numVertices() - 1){ // keep going until all vertices are added. make sure we don't exceed more than the number of vertices in graph. -1 to numVertices() because one was already added (source vertex)
            ...
            if(cheapestEdge == null) { // means that all vertices have been added
                break;
            }
    ...
    /*
        method name: getCheapestAddedVertex() - method that returns the edge with the cheapest cost of un added edges by using the table and by keeping track of the minCost
        iterates through the vertices and each of their neighbors using getFirstEdge() and a while loop and checks if edge is added
        along with checking if it's cheaper than the current minCost, if so set minCost to the current edges cost and continue iterating
        by the end of the loop, cheapest cost along with the edge it belongs to is found. table is then updated accordingly.
     */
